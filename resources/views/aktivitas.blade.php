...
<table class="table">
  <thead class="thead-dark">
    ...
    <tr>
      ...
      <td><a href="{{url('/aktivitas')}}/{{$aktivitas->id}}">{{$aktivitas->nama_kegiatan}}</a></td>
      <td>{{$aktivitas->jenis_kegiatan}}</td>
      <td>{{$aktivitas->tanggal}}</td>
      <td>{{$aktivitas->deskripsi_kegiatan}}</td>
      <td>{{$aktivitas->lokasi_kegiatan}}</td>
       <td style="display: flex;"><a class="btn btn-sm btn-primary" href="{{ route('aktivitas.edit',[$aktivitas->id])}}">Edit </a>
